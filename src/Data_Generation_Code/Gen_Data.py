...
i = 1
while True:
    # path = f"Data/Generated_datasets_and_input_instance/Instance_{i}/Config.txt"
    path = f"../../Data/Generated_datasets_and_input_instance/Instance_{i}/Config.txt"
    try:
...
cities_df = pd.read_csv("../../Data/Raw_data/indian-cities.csv")
...
def generate_data(data):
    ...
    NUM_COMPANIES = int(data[1].strip())
    NUM_OPERATORS = int(data[2].strip())
    NUM_STORES_PER_OPERATOR = int(data[3].strip())
    NUM_CUSTOMERS = int(data[4].strip())
    NUM_VEHICLES = int(data[5].strip())
    NUM_VEHICLE_TYPES = int(data[6].strip())
    NUM_ITEM_TYPES_FOR_INVENTORY = int(data[7].strip())
    NUM_SHIPMENT_REQUESTS = int(data[8].strip())
    NUM_VEHICLE_OPERATOS = int(data[9].strip())
    NUM_VEHICLE_OWNERS = int(data[10].strip())
    ...
    # Create DataFrame from city_list
    city_df = pd.DataFrame([vars(city) for city in city_list])
    ...
    def generate_companies():
        """Generate company data with random attributes."""
        companies = []

        for i in range(NUM_COMPANIES):
            # Select cities where at least one operator exists
            ...
            company = Company(
                company_id=i + 1,
                name=f"Company_{i + 1}",
                contact_email=f"help@company{i + 1}.com",
                contact_phone=f"+91-{
                    random.randint(9000000000, 9999999999)}",
                contact_person_name=f"Person_{i + 1}",
                registered_name=f"Registered_Company_{i + 1}",
                registered_address=f"Address_{i + 1}",
                Max_number_of_orders_each_vehicle_can_take=1,
                operates_in=operates_in,
                start_time=datetime.combine(
                    datetime.today(), datetime.strptime("09:00", "%H:%M").time()),
                end_time=datetime.combine(
                    datetime.today(), datetime.strptime("21:00", "%H:%M").time()),
                list_of_operators=[j + 1 for j in range(NUM_OPERATORS)]
            )

            # Add the company to the companies list
            companies.append(company)

            # Add the company to the list_of_companies in the cities it operates in
            for city_id in operates_in:
                city_list[city_id -
                          1].list_of_companies.append(company.company_id)
    ...
    def generate_operators():
        ...
        for i in range(NUM_OPERATORS):
            # Assign the operator to operate in random cities
            ...
            for city in city_list:
                if city.list_of_companies:
                    operator.operates_in.append(city.city_id)
                    city.list_of_operators.append(operator.operator_id)
    ...
    def generate_stores():
        ...
        for operator in operators:
            for city_id in operator.operates_in:
                store_points = generate_random_points(
                    Point(city_list[city_id - 1].center_latitude,
                          city_list[city_id - 1].center_longitude),
                    radius=city_list[city_id - 1].radius,
                    num_points=NUM_STORES_PER_OPERATOR
                )

                # Create stores for this operator in this city
                for k in range(NUM_STORES_PER_OPERATOR):
                    store = Store(
                        ...
                        contact_email=f"store{
                            store_id}@{city_list[city_id - 1].name.lower()}.com",
                        contact_mobile=random.randint(
                        ...
                        end_time=datetime.combine(
                            datetime.today(), datetime.strptime("21:00", "%H:%M").time()),
                        list_of_items=[],
                        inventory_capacity=random.uniform(100, 500)
                    )

                    operator.list_of_stores.append(store_id)
                    stores.append(store)
    ...
    def generate_customers():
        ...
        for i in range(1, NUM_CUSTOMERS + 1):
            ...
            angle = random.uniform(0, 360)
            destination = geodesic(kilometers=distance).destination(
                Point(city.center_latitude, city.center_longitude), angle)
            customer = Customer(
                customer_id=i,
                city_id=city_id,
    ...
    def generate_vehicles():
        ...
        for i in range(1, NUM_VEHICLES + 1):
            vehicle = Vehicle(
                vehicle_id=i,
                vehicle_type_id=random.choice(
                    range(1, NUM_VEHICLE_TYPES + 1)),
                owner=f"Owner_{i}",
                driver=f"Driver_{i}",
                vehicle_operator=random.choice(
                    range(1, NUM_OPERATORS + 1)),
                fuel_type=random.choice(["Petrol", "Diesel", "Electric"]),
                vehicle_status=random.choice(
                    ["Available", "In Use", "Under Maintenance"]),
                number_plate=f"AB-{random.randint(10, 99)
    ...
    def generate_vehicle_types():
        ...
        vehicle_type_names = ["two wheeler", "three wheeler closed",
                              "three wheeler open", "four wheeler closed", "four wheeler open"]
        vehicle_types = []
        for i in range(1, vehicle_type_names.__len__() + 1):
            vehicle_type = VehicleType(
                id=i,
                name=vehicle_type_names[i-1],
                weight_carrying_capacity=round(
                    random.uniform(500, 2000), 2),
    ...
    def generate_vehicle_operators():
        ...
        for i in range(1, NUM_VEHICLE_OPERATOS+1):
            vehicle_operator = VehicleOperator(
                ...
                Aadhar=f"{random.randint(1000, 9999)} {random.randint(1000, 9999)} {
    ...
    # Populate vehicle operators list
    vehicle_operators = generate_vehicle_operators()
            ...
            for i in range(0, len(city.list_of_operators)):
                for j in range(1, 100):
                    driver = Driver(
                        ...
                        aadhar=f"{random.randint(1000, 9999)} {random.randint(1000, 9999)} {
                            random.randint(1000, 9999)}",
                        dl_number=f"DL-{random.randint(1000, 9999)
                                        }-{random.randint(100000, 999999)}",
                        dl_for_vehicle_types=random.sample(
                            range(1, NUM_VEHICLE_TYPES + 1), random.randint(1, 3)),
    ...
    def generate_item_type_for_inventory():
        ...
        for i in range(1, NUM_ITEM_TYPES_FOR_INVENTORY + 1):
            item = InventoryItem(
                ...
                dimensions=(round(random.uniform(0.1, 1), 2), round(
                    random.uniform(0.1, 1), 2), round(random.uniform(0.1, 1), 2)),
                special_handling_requirements=random.choice(
                    ["Fragile", "Perishable", "Hazardous", "None"])
            )
            items.append(item)
        return items

    # Populate inventory items list
    item_type_for_inventory = generate_item_type_for_inventory()
        ...
        for i in range(NUM_SHIPMENT_REQUESTS):
            ...
            shipment = ShipmentRequest(
                ...
                facility_id=random.choice(
                    range(1, NUM_STORES_PER_OPERATOR + 1)),
                customer_id=random.choice(range(1, NUM_CUSTOMERS + 1)),
                vehicle_id=random.choice(range(1, NUM_VEHICLES + 1)),
                pickup_time=pickup_time,
                delivery_time=delivery_time,
                items_shipped=random.sample(
                    range(1, NUM_ITEM_TYPES_FOR_INVENTORY + 1), random.randint(1, 5)),
    ...
    def inventory():
        ...
        for i in range(len(stores)):
            ...
            for item in item_type_for_inventory:
                ...
                inventory.number_of_items_per_type.append(
    ...
    def generate_available_drivers(N=1000):
        ...
        for city in city_list:
            ...
            list_of_companies = city.list_of_companies
            ...
            for driver_location in driver_locations:
                ...
                available_driver = AvailableDriver(
                    ...
                    driver_id=drivers[((driver_id - 1) + \
                                       len(drivers)) % len(drivers)].id,
                    city_id=city_id,
                    # Cycle through vehicles
                    vehicle_id=vehicles[(vehicle_id - 1) %
                                        len(vehicles)].vehicle_id,
                    name=f"Driver_{driver_id}",
                    latitude=driver_latitude,
                    longitude=driver_longitude,
                    company_id=random.choice(list_of_companies),
                    is_available=True,
                    availability_time=datetime.combine(
                        datetime.today(), datetime.strptime("08:00", "%H:%M").time()),
                    max_delivery_radius=city.radius,
  ...
  # Populate available drivers list
    ...
    def generate_requests(N):
        ...
        for city in city_list:
            ...
            center_longitude = city.center_longitude
            list_of_companies = city.list_of_companies
            list_of_operators = city.list_of_operators
            list_of_stores = [store.store_id for store in stores if store.city_id == city.city_id]
            ...
            for i in range(1, number_of_requests_in_city + 1):
                delivery_items = []
                max_number_of_items = random.randint(
                    1, len(item_type_for_inventory) + 1)
                ...
                request = Request(
                    ...
                    # assures there is an id, better than writing customer_id directly
                    Customer_id=customers[customer_id - 1].customer_id,
                    company_id=random.choice(list_of_companies),
                    operator_id=random.choice(list_of_operators),
                    store_id=random.choice(list_of_stores),
                    Request_placing_time=datetime.combine(
                        datetime.today(), datetime.strptime("09:00", "%H:%M").time()),
    ...
    companies_df = pd.DataFrame([vars(company) for company in companies])
    ...
    vehicle_types_df = pd.DataFrame(
        [vars(vehicle_type) for vehicle_type in vehicle_types])
    inventory_df = pd.DataFrame([vars(inventory)
                                for inventory in inventory_details])
    available_drivers_df = pd.DataFrame(
        [vars(driver) for driver in available_drivers])
    requests_df = pd.DataFrame([vars(request) for request in requests])
    vehiclle_operators_df = pd.DataFrame(
        [vars(vehicle_operator) for vehicle_operator in vehicle_operators])
    vehicle_owners_df = pd.DataFrame(
        [vars(vehicle_owner) for vehicle_owner in vehicle_owners])
    active_requests_df = pd.DataFrame()
    unserviced_requests_df = pd.DataFrame()
    completed_requests_df = pd.DataFrame()

    # Check if output directory exists
    # output_directory = f"Data/Generated_datasets_and_input_instance/Instance_{Index_Of_Config}/Generated_data"
    output_directory = f"../../Data/Generated_datasets_and_input_instance/Instance_{Index_Of_Config}/Generated_data"
    ...
    try:
        ...
        companies_df.to_csv(f"{output_directory}/companies.csv", index=False)
        ...
        items_df.to_csv(
            f"{output_directory}/item_type_for_inventory.csv", index=False)
        ...
        vehicle_types_df.to_csv(
            f"{output_directory}/vehicle_types.csv", index=False)
        ...
        available_drivers_df.to_csv(
            f"{output_directory}/available_drivers.csv", index=False)
        active_requests_df.to_csv(
            f"{output_directory}/active_requests.csv", index=False)
        unserviced_requests_df.to_csv(
            f"{output_directory}/unserviced_requests.csv", index=False)
        completed_requests_df.to_csv(
            f"{output_directory}/completed_requests.csv", index=False)
        vehiclle_operators_df.to_csv(
            f"{output_directory}/vehicle_operators.csv", index=False)
        vehicle_owners_df.to_csv(
            f"{output_directory}/vehicle_owners.csv", index=False)
...
for data in config_data:
    generate_data(data)
    print(f"Data has been saved to the respective CSV files for Instance {
          Index_Of_Config}.")
    Index_Of_Config += 1
...
# Assumptions
# 1. Each company is at max associated with 10 cities
# 2. Each company has every operator
# 3. Every operator will be there in a city where there is a company
